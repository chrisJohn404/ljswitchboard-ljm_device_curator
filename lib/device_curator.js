...
function device(useMockDevice) {
	...
	function isDemoModeActiveFromOpenParameters(openParameters) {
		if(openParameters.identifier === 'LJM_DEMO_MODE') {
			return true;
		} else {
			return false;
		}
	...
	var saveAndLoadAttributes = function(openParameters) {
		var saveAndLoad = function() {
			...
			self.savedAttributes.isConnected = true;
			self.savedAttributes.isDemoMode = isDemoModeActiveFromOpenParameters(openParameters);
			this.allowReconnectManager = false;

			self.getHandleInfo()
			.then(function(info) {
				console.log('Saving Handle Info', info, openParameters, self.getDevice());
